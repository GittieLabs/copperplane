...
class InstallerBrandAssetsTests(unittest.TestCase):
    """The DMG background is a copy of a generated asset, and copies fork.

    Same reasoning as the app's and the docs site's marks: `brand/` is
    generated by `brand/tools/`, the Tauri bundler needs the file beside
    `tauri.conf.json`, and a copy nobody checks goes stale the first time the
    kit is regenerated -- silently, because a DMG background looks fine until
    somebody compares it.
    """

    SOURCE = os.path.join(REPO_ROOT, "brand", "png", "dmg-background.png")
    SHIPPED = os.path.join(REPO_ROOT, "core", "tauri-rust", "dmg", "background.png")

    @unittest.skipUnless(os.path.exists(SOURCE), "the brand kit is not present")
    def test_501_the_shipped_background_is_the_generated_one(self):
        with open(self.SOURCE, "rb") as a, open(self.SHIPPED, "rb") as b:
            self.assertEqual(a.read(), b.read())

    def test_502_the_config_points_at_it_and_the_layout_matches_the_drawing(self):
        """The background is drawn for one specific window size and two
        specific icon positions. If either moves, the trace no longer points
        from the app to the folder -- it points at nothing, and nothing fails.
        """
        import json
        with open(os.path.join(REPO_ROOT, "core", "tauri-rust", "tauri.conf.json"), encoding="utf-8") as handle:
            dmg = json.load(handle)["bundle"]["macOS"]["dmg"]

        self.assertEqual(dmg["background"], "dmg/background.png")
