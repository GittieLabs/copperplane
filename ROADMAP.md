...
confirmation gate (`CTX-108.3`'s "Inject into Board" button still has no review step) is likewise
unwired — `SPEC-108`'s re-scoped injection action or `SPEC-308`'s job per `PRODUCT-PLAN.md` §5.2.
...
| An LLM hallucinates a plausible-but-wrong footprint that reaches a real board | A wasted PCB spin; the fastest way to lose a hardware engineer's trust permanently | SPEC-202 validation layer + ✅ SPEC-204's confirmation gate real in the daemon (CTX-204.1) — not yet closed end-to-end: CTX-108.3's "Inject into Board" UI button still calls kicad.inject_component directly, unwired to the gate |
